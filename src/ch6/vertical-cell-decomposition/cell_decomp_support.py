...
class VerticalCellDecomposition:
    ...
    def calc_face_split(edge_list, C_pt, angles=[np.pi/2, -np.pi / 2]):
        ...
        for angle in angles:
            for e in edge_list:
                ...
                if VerticalCellDecomposition.test_for_intersection(A.get_point_coordinate(), B.get_point_coordinate(), C_pt, angle):
                    split_vertices.append(VerticalCellDecomposition.get_intersection_pt(A.get_point_coordinate(), B.get_point_coordinate(), C_pt, angle))
    ...
    def calculate_free_points(edge_list, boundary_vertex):
        ...
        for va in boundary_vertex.angles:
            intersections = [[mfn.euclidean_dist(pt, i), i] for i in VerticalCellDecomposition.calc_face_split(edge_list, pt, [va])]
    ...
    def check_for_free_path(edge_list, origin, angle, distance):
        distkey = lambda x: x[0]
        intersections = [[mfn.euclidean_dist(origin, i), i] for i in VerticalCellDecomposition.calc_face_split(edge_list, origin, [angle])]
        intersections = sorted(intersections, key=distkey)
    ...
    def process_layer(boundary_event, edge_list):
        sortkey = lambda bv: bv.vertex.rank
        rank = sortkey(boundary_event)
    ...
    def build_roadmap(dcel):
        ...
        free_points = VerticalCellDecomposition.calculate_free_points(valid_edges, boundary_events[0])
        pairs = []
        curr_layer = []
        last_layer = []
        ...
        for i in range(1, len(boundary_events)):
            ...
            valid_edges = VerticalCellDecomposition.get_active_edges(global_edge_list, rank)
            last_active_edges = VerticalCellDecomposition.get_past_edges(global_edge_list, rank)
            
            free_points = VerticalCellDecomposition.calculate_free_points(valid_edges, bv)
            if len(free_points) == 2:
                last_layer.append(mfn.pol2car(bv.vertex.get_point_coordinate(), 5, np.pi))
                free_points.append(mfn.pol2car(bv.vertex.get_point_coordinate(), 4, np.pi))
                last_layer.append(mfn.pol2car(bv.vertex.get_point_coordinate(), 1, np.pi))
            for j in range(len(free_points)):
                ...
                for k in range(len(last_layer)):
                    ...
                    if VerticalCellDecomposition.check_for_free_path(last_active_edges, pt, theta, radius):# and check_for_free_path(last_active_edges, pt, theta, radius):
